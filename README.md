...
## Step 5: Install oh-my-claudecode (OMC)

> Repo: [Yeachan-Heo/oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode) | npm: [oh-my-claude-sisyphus](https://www.npmjs.com/package/oh-my-claude-sisyphus)

Requires: Claude Code CLI + Claude Max/Pro subscription or Anthropic API key.
...
This generates `CLAUDE.md`, `.omc-config.json`, the HUD status line, and plugin hooks.

Enable native agent teams in `settings.json`:
...
Or manually: `/plugin install oh-my-claudecode` then `/oh-my-claudecode:omc-setup`. Run `/oh-my-claudecode:doctor` if issues persist.
...
```jsonc
{
  ...
  // OMC HUD status line (update username to match your machine)
  "statusLine": {
    "type": "command",
...
The OMC HUD displays real-time status information in Claude Code's status bar (mode, agent activity, token usage, etc.).
...
`hud/omc-hud.mjs` is a thin wrapper that loads the HUD renderer from the OMC plugin cache:

1. Looks for the latest version in `~/.claude/plugins/cache/omc/oh-my-claudecode/<version>/dist/hud/index.js`
2. Falls back to local development paths (`~/Workspace/oh-my-claudecode/dist/hud/index.js`)
3. If neither found, prints an actionable error message

The HUD is auto-generated by `/oh-my-claudecode:omc-setup`. To customize display options:

```
/oh-my-claudecode:hud
```

This lets you configure layout, presets, and which display elements to show (mode indicators, token counters, agent status, etc.).
...
Installed LSP servers used by the `typescript-lsp` plugin and OMC's LSP tools (`lsp_hover`, `lsp_goto_definition`, `lsp_diagnostics`, etc.):
...
**Tracked**: `agents/`, `commands/`, `profiles/`, `skills/`, `hooks/`, `scripts/`, `settings.json`, `settings.local.json`, `.ignore`, `mcp-proxy-servers.example.json`, `.gitignore`, `README.md`

**Not tracked** (secrets/ephemeral): `CLAUDE.md` (generated by OMC), `mcp-proxy-servers.json` (contains API keys), `debug/`, `transcripts/`, `projects/`, `cache/`, `plugins/`, `hud/`, etc.
...
# 9. Launch Claude Code and install OMC
claude
# Inside Claude Code:
#   /plugin marketplace add https://github.com/Yeachan-Heo/oh-my-claudecode
#   /plugin install oh-my-claudecode
