...
All platforms benefit from JUnit output for native test result display. Each reporter list is a named const typed as `ReporterDescription[]`; the ternary picks one so `defineConfig` receives a plain value.
...
Missing JUnit reporter or `PublishTestResults@2` task. Add `['junit', { outputFile: 'results/junit.xml' }]` to the `reporter` list (see [JUnit Reporter Config](#junit-reporter-config)) and publish the file:
