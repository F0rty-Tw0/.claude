...
**Symptom:** `page.goto(appUrl)` is immediately 302'd to `/error?er=2` (or analogous error). The trace shows ONE navigation, the inner JS never runs, no expected XHRs fire.
...
```ts
export const gotoAppUrl = async (
    page: Page,
    accountSettings: { baseUrl: string; accountId: string },
    resourceId: string,
    extraQuery?: string,
): Promise<string> => {
    const baseUrl = `${accountSettings.baseUrl}/?account=${accountSettings.accountId}&resource=${resourceId}`;
...
Adopt this everywhere you visit a freshly-created resource: `await gotoAppUrl(page, account, id)` instead of `await page.goto(getAppUrl(...))`.
