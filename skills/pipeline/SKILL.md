...
The pipeline skill enables chaining multiple agents together in defined workflows where the output of one agent becomes
the input to the next. This creates powerful agent pipelines similar to Unix pipes but designed for AI agent
orchestration.
...
1. `parallel(external-researcher, explore)` - External docs + internal code
...
```
/pipeline [explore, external-researcher] -> architect -> executor "implement OAuth"
```
...
Pipelines maintain state in `.claude/local/pipeline-state.json`:
...
→ Triggers: parallel(external-researcher, explore) → architect → writer
...
**Check:** `.claude/local/pipeline-state.json` for current stage **Fix:** Resume with `/pipeline resume` or cancel and
restart
...
rm -f .claude/local/state/pipeline-state.json
```
