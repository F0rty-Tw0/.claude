...
Ultrapilot is the parallel evolution of autopilot. It decomposes your task into independent parallelizable subtasks,
assigns non-overlapping file sets to each worker, and runs them simultaneously.
...
**Data Structure:** `.claude/local/state/ultrapilot-ownership.json`
...
**Location:** `.claude/local/ultrapilot-state.json`
...
**Location:** `.claude/local/state/ultrapilot-ownership.json`

Tracks which worker owns which files (see Phase 2 example above).

### Progress Tracking

**Location:** `.claude/local/ultrapilot/progress.json`
...
4. **Monitor progress** - Check `.claude/local/ultrapilot/progress.json`
...
- Review `.claude/local/ultrapilot/decomposition.json` for details

**Worker hangs?**

- Check worker logs in `.claude/local/logs/ultrapilot-worker-N.log`
- Cancel and restart that worker
- May indicate file ownership issue

**Integration conflicts?**

- Review `.claude/local/ultrapilot-state.json` conflicts array
...
**Location:** `.claude/local/ultrapilot/custom-decomposition.json`
...
rm -f .claude/local/state/ultrapilot-state.json
