...
>
> ```bash
> teleport #123          # Create worktree for issue/PR
> teleport my-feature    # Create worktree for feature
> teleport list          # List worktrees
> ```
>
> See [Teleport Command](#teleport-command) below for details.

Automate isolated development environments using git worktrees and tmux sessions with Claude Code. Enables parallel work
across multiple tasks, projects, and repositories.

## Commands

| Command                 | Description          | Example                               |
| ----------------------- | -------------------- | ------------------------------------- |
| `review <ref>`          | PR review session    | `/psm review myproject#123`           |
| `fix <ref>`             | Issue fix session    | `/psm fix myproject#42`               |
| `feature <proj> <name>` | Feature development  | `/psm feature myproject add-webhooks` |
| `list [project]`        | List active sessions | `/psm list`                           |
| `attach <session>`      | Attach to session    | `/psm attach myproject:pr-123`        |
| `kill <session>`        | Kill session         | `/psm kill myproject:pr-123`          |
...
- **Alias**: `myproject#123` (requires `~/.psm/projects.json`)
- **Full**: `owner/repo#123`
...
**Important:** The `repo` field is still required for cloning the git repository. Jira tracks issues, but you work in a
git repo.
...
PSM only recognizes `PROJ-123` format as Jira when `PROJ` is explicitly configured as a `jira_project` in your aliases.
This prevents false positives from branch names like `FIX-123`.
...
| --------- | ------------------------- | -------------------------------------- |
| PR Review | `psm:myproject:pr-123`    | `~/.psm/worktrees/myproject/pr-123`    |
| Issue Fix | `psm:myproject:issue-42`  | `~/.psm/worktrees/myproject/issue-42`  |
| Feature   | `psm:myproject:feat-auth` | `~/.psm/worktrees/myproject/feat-auth` |
...
9. **Output session info**:
   ...
   Session ready!
     ...
     ID: myproject:pr-123
     Worktree: ~/.psm/worktrees/myproject/pr-123
     Tmux: psm:myproject:pr-123

   To attach: tmux attach -t psm:myproject:pr-123
   ```
...
4. **Format output**:
   ...
   -------------------|---------|----------|---------------------------
   myproject:pr-123        | review  | active   | ~/.psm/worktrees/myproject/pr-123
   myproject:issue-42      | fix     | detached | ~/.psm/worktrees/myproject/issue-42
   ```
...
5. **Report**:
   ```
   Cleanup complete:
     Removed: myproject:pr-123 (merged)
     Removed: myproject:issue-42 (closed)
     Kept: myproject:feat-auth (active)
   ```
...
3. **Show status**:
   ```
   Current Session: myproject:pr-123
   Type: review
...
The `teleport` command provides a lightweight alternative to full PSM sessions. It creates git worktrees without tmux
session management — ideal for quick, isolated development.
...
# Remove a worktree
teleport remove issue/my-repo-123
teleport remove --force feat/my-repo-my-feature
```
...
| `--path <path>`   | Custom worktree root directory                    | `~/Workspace/worktrees/` |
