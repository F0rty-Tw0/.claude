 ...
 * relationship types, and graph-based analysis.
 */
 ...
 */
export function getLocalizedRelationType(type: RelationshipType): string {
...
export function generateRelationshipProfile(char1Name: string, char2Name: string): string {
  ...
  profile += `**관계 유형**: ${getLocalizedRelationType(relationship.type)}\n`;
  ...
  web.nodes.forEach(node => {
    ...
    if (connections.length > 0) {
      ...
      connections.forEach(conn => {
        ...
        map += `  ${symbol} ${conn.otherCharacter} (${getLocalizedRelationType(conn.relationship.type)})\n`;
      });
      map += '\n';
    }
  });
