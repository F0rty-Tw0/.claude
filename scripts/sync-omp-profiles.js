...
const MODEL_MAP = {
  'openai-codex/sol': 'anthropic/claude-opus-5',
  'openai-codex/terra': 'anthropic/claude-sonnet-5',
...
if (!files.length) {
  console.error(`no agent files in ${SRC}`);
  process.exit(2);
}

const toAnthropic = (s) =>
  Object.entries(MODEL_MAP).reduce(
    (acc, [k, v]) => acc.replaceAll(`model: ${k}`, `model: ${v}`),
    s,
  );
...
for (const dir of [...TARGETS.verbatim, ...TARGETS.anthropic]) {
  ...
  for (const f of fs.readdirSync(dir).filter((x) => x.endsWith('.md'))) {
    if (!files.includes(f))
      console.log(`ORPHAN (not in source, left alone): ${path.join(dir, f)}`);
  }
}
console.log(
  check
    ? `${drift} drifted file(s)`
    : `synced ${files.length} agents, ${written} file(s) written`,
);
