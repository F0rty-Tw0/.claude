...
- `/plan` skill = writes the plan; `architect` = system-design review; `critic` = tears a plan apart.
- `code-reviewer` = whole-step review against the plan; the panel (`security-reviewer` / `performance-reviewer` / `quality-reviewer`) = deep single-dimension passes.
